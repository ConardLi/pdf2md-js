// filepath: c:\code\pdf2md-js\src\utils.ts
/**
 * 工具函数模块，提供常用的辅助功能
 */
...
export type RECT = GeoJSON.Feature<GeoJSON.Polygon, GeoJSON.GeoJsonProperties>;

/**
 * 创建矩形几何对象
 * @param bounds [x0, y0, x1, y1] 表示矩形的左上和右下坐标
 * @returns turf几何对象
 */
export const createRect = (bounds: number[]): RECT => {
 ...
 * @returns 距离
 */
export const getDistance = (rect1: RECT, rect2: RECT, buffer = 0.1): number => {
  // 直接使用几何计算来获取距离
 ...
 * @returns 是否接近
 */
export const isNear = (rect1: RECT, rect2: RECT, distance = 20): boolean => {
  try {
    // 检查矩形是否有效
 ...
 * @returns 是否水平接近
 */
export const isHorizontalNear = (rect1: RECT, rect2: RECT, distance = 100): boolean => {
  try {
    // 检查矩形是否有效
    if (!rect1 || !rect2) return false;
 ...
 * @returns 合并后的矩形
 */
export const unionRects = (rect1: RECT | null, rect2: RECT | null): RECT | null => {
  try {
    // 检查矩形是否有效
 ...
 * @returns 随机文件名
 */
export const generateRandomFileName = (prefix = 'img', extension = 'png'): string => {
 ...
 * @returns 是否有效
 */
export const isValidRect = (rect: any): boolean => {
  try {
    // 检查是否为空
