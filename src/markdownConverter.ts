 ...
 * @param imageInfos 图像信息数组 [{ pageImage, rectImages }, ...]
...
export const convertImagesToMarkdown = async (imageInfos: ImageInfo[], outputDir: string, options: ConversionOptions): Promise<string> => {
  ...
  for (let pageIndex = 0; pageIndex < imageInfos.length; pageIndex++) {
    ...
    // 处理页面上的每个区域图像
    const tasks = rectImages.map(async (rectImage: string, rectIndex: number) => {
    ...
    // 并行处理图像（根据并发度）
    const results: Array<{ rectIndex: number; markdown: string }> = [];
