...
import type { PDFPageProxy, TextItem } from 'pdfjs-dist/types/src/display/api.d.ts';
import { createRect, RECT } from './utils';
import { mergeRects, adsorbRectsToRects, filterSmallRects } from './rectProcessor';
 ...
 * @returns 识别出的区域坐标数组 [[x0, y0, x1, y1], ...]
 */
export const parseRects = async (page: PDFPageProxy): Promise<number[][]> => {
  console.log('\n开始解析页面区域...');
  // 获取页面尺寸
  ...
  // 创建文本区域的矩形列表
  const textRects = textContent.items.map((_item) => {
    // @TODO 处理 TextMarkedContent
    const item = _item as TextItem; // 强制转换为TextItem类型
  ...
  for (let i = 0; i < operatorList.fnArray.length; i++) {
    ...
    if (fn === OPS.paintImageXObject) {
      ...
      if (i > 0 && operatorList.fnArray[i - 1] === OPS.transform) {
 ...
 * @returns 每个页面的区域信息 [{ pageIndex, rects: [[x0, y0, x1, y1], ...] }, ...]
 */
export const parsePdfRects = async (pdfPath: string): Promise<PageRect[]> => {
  // 加载PDF文档
  const data = new Uint8Array(await fs.readFile(pdfPath));
  const pdf = await getDocument({ data }).promise;

  const pageCount = pdf.numPages;
  const results: PageRect[] = [];
